...
parser.add_argument(
    "--i_field",
    help="Registration to our fake reference image, provided by synthmorph. Will be computed if if does not exist",
)
...
parser.add_argument(
    ...
    type=float,
    default=0.05,
    help="(optional) Value of SynthMorph regularizer. Must be between 0 and 1. Default is 0.05.",
)
parser.add_argument(
    "--resolution",
    type=float,
...
RAS = torch.stack(
    [
        ...
        + aff[0, 2] * IJKim[2]
        + aff[0, 3]
        + torch.squeeze(FIELD[..., 0]),
        aff[1, 0] * IJKim[0]
        + aff[1, 1] * IJKim[1]
        + aff[1, 2] * IJKim[2]
        + aff[1, 3]
        + torch.squeeze(FIELD[..., 1]),
        aff[2, 0] * IJKim[0]
        + aff[2, 1] * IJKim[1]
        + aff[2, 2] * IJKim[2]
        + aff[2, 3]
        + torch.squeeze(FIELD[..., 2]),
    ]
)
del FIELD
...
tissue_index, grouping_labels, label_list, number_of_gmm_components = (
    relab.get_tissue_settings(
        ...
        os.path.join(
            BASE_PATH,
            "data_simplified",
            "combined_atlas_labels_" + args.gmm_mode + ".yaml",
        ),
        os.path.join(
            BASE_PATH,
            "data_simplified",
            "combined_aseg_labels_" + args.gmm_mode + ".yaml",
        ),
        os.path.join(
            BASE_PATH, "data_simplified", "gmm_components_" + args.gmm_mode + ".yaml"
        ),
        aseg_label_list,
    )
)
...
_, grouping_labels_right, _, _ = relab.get_tissue_settings(
    os.path.join(BASE_PATH, "data_simplified", "atlas_names_and_labels.yaml"),
    os.path.join(
        BASE_PATH, "data_simplified", "combined_atlas_labels_" + args.gmm_mode + ".yaml"
    ),
    os.path.join(
        BASE_PATH, "data_simplified", "combined_aseg_labels_" + args.gmm_mode + ".yaml"
    ),
    os.path.join(
        BASE_PATH, "data_simplified", "gmm_components_" + args.gmm_mode + ".yaml"
    ),
...
aff_A = np.diag(
    [0.2, 0.2, 0.2, 1]
)  # assumes left hemisphere; we'll deal with rest later
...
for h in range(2):
    ...
    I_r[M_r < 0.5] = mu_bg
    RAS_r, _ = my.torch_resize(
        RAShemis[h].permute([1, 2, 3, 0]), aff[h], resolution, device, dtype=dtype
    )
    ...
    if h == 1:  # right hemisphere; easy!
        T[:, 0] = -T[:, 0]
    Iskip = (
        T[0, 0] * RAS_r[::skip, ::skip, ::skip, 0]
        + T[0, 1] * RAS_r[::skip, ::skip, ::skip, 1]
        + T[0, 2] * RAS_r[::skip, ::skip, ::skip, 2]
        + T[0, 3]
    )
    Jskip = (
        T[1, 0] * RAS_r[::skip, ::skip, ::skip, 0]
        + T[1, 1] * RAS_r[::skip, ::skip, ::skip, 1]
        + T[1, 2] * RAS_r[::skip, ::skip, ::skip, 2]
        + T[1, 3]
    )
    Kskip = (
        T[2, 0] * RAS_r[::skip, ::skip, ::skip, 0]
        + T[2, 1] * RAS_r[::skip, ::skip, ::skip, 1]
        + T[2, 2] * RAS_r[::skip, ::skip, ::skip, 2]
        + T[2, 3]
    )

    Iskip *= 2.0 / (A.shape[0] - 1.0)
    Iskip -= 1.0
    Jskip *= 2.0 / (A.shape[1] - 1.0)
    Jskip -= 1.0
    Kskip *= 2.0 / (A.shape[2] - 1.0)
    Kskip -= 1.0

    # Careful with grid_sample switching the order of the dimensions (go figure...)
    priors = grid_sample(
        A.permute([3, 0, 1, 2])[None, ...],
        torch.stack([Kskip, Jskip, Iskip], axis=-1)[None, ...],
        align_corners=True,
    )
    ...
    for em_it in range(100):
        ...
        if (torch.sum(torch.isnan(means)) > 0) or (
            torch.sum(torch.isnan(variances)) > 0
        ):
...
for h in range(2):
    ...
    GAUSSIAN_LHOODS.append(
        torch.zeros(
            [*I_full.shape, sum(number_of_gmm_components)], dtype=dtype, device=device
        )
    )
    for c in range(sum(number_of_gmm_components)):
        ...
        GAUSSIAN_LHOODS[h][..., c] = (
            1.0
            / torch.sqrt(2 * math.pi * variances_both_sides[h][c])
            * torch.exp(
                -0.5
                * torch.pow(I_full - means_both_sides[h][c], 2.0)
                / variances_both_sides[h][c]
            )
            + 1e-9
        )
    del I_full, M_full
...
for h in range(2):
    normalizers.append(
        torch.zeros(GAUSSIAN_LHOODS[h].shape[:-1], dtype=dtype, device=device)
    )
    # normalizers[h][:] = 1e-9
    gaussian_number = 0
    for c in range(A.shape[-1]):
        prior = grid_sample(
            A[None, None, ..., c],
            torch.stack([K[h], J[h], I[h]], axis=-1)[None, ...],
            align_corners=True,
        )[0, 0, ...]
        if c == 0:  # background
            prior[
        ...
        for g in range(number_of_gmm_components[c]):
            lhood += (
                weights_both_sides[h][gaussian_number]
                * GAUSSIAN_LHOODS[h][..., gaussian_number]
            )
            gaussian_number += 1
...
for n, (prior_indices, prior_values) in enumerate(label_loader):
    ...
    for h in range(2):
        ...
        if n == 0:
            ...
            Irescaled = I[h] * ((A.shape[0] - 1) / (max_x - min_x)) + (
                (A.shape[0] - 1 - min_x - max_x) / (max_x - min_x)
            )
            Jrescaled = J[h] * ((A.shape[1] - 1) / (max_y - min_y)) + (
                (A.shape[1] - 1 - min_y - max_y) / (max_y - min_y)
            )
            Krescaled = K[h] * ((A.shape[2] - 1) / (max_z - min_z)) + (
                (A.shape[2] - 1 - min_z - max_z) / (max_z - min_z)
            )
            mask = Irescaled >= (-1)
            mask &= Irescaled <= 1
            mask &= Jrescaled >= (-1)
        ...
        if skip_this_label == False:
            ...
            prior_resampled = grid_sample(
                prior[None, None, ...],
                torch.stack(
                    [Krescaled[lr_crop], Jrescaled[lr_crop], Irescaled[lr_crop]],
                    axis=-1,
                )[None, ...],
                align_corners=True,
            )[0, 0, ...]
            ...
            lhood = torch.sum(
                GAUSSIAN_LHOODS[h][:, :, :, gaussian_numbers]
                * weights_both_sides[h][None, None, None, gaussian_numbers],
                3,
            )
...
my.MRIwrite(
    seg[0].detach().cpu().numpy().astype(np.uint16),
    aff_l,
    output_dir + "/seg_left.nii.gz",
)
_, aff_r = my.torch_resize(Iim[1], aff[1], resolution, device, dtype=dtype)
my.MRIwrite(
    seg[1].detach().cpu().numpy().astype(np.uint16),
    aff_r,
    output_dir + "/seg_right.nii.gz",
)
if args.write_rgb:
    my.MRIwrite(
        seg_rgb[0].detach().cpu().numpy().astype(np.uint8),
        aff_l,
        output_dir + "/seg_left.rgb.nii.gz",
    )
    my.MRIwrite(
        seg_rgb[1].detach().cpu().numpy().astype(np.uint8),
        aff_r,
        output_dir + "/seg_right.rgb.nii.gz",
    )
