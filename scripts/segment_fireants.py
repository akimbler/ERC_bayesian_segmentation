...
FIREANTS_PATH = os.path.join(
    os.environ.get("FREESURFER_HOME"), "python/packages/ERC_bayesian_segmentation/ext"
)
...
parser.add_argument(
    "--i_seg",
    help="SynthSeg of image to segment (must include parcels). Will be computed if it does not exist",
)
parser.add_argument(
    "--i_field",
    help="Registration to MNI, provided by EasyReg, for left-right division etc. Will be computed if if does not exist",
)
parser.add_argument("--atlas_dir", help="Atlas directory")
parser.add_argument("--side", help="Hemisphere to segment (left or right).")
parser.add_argument(
    "--bf_mode",
    help="bias field basis function: dct, polynomial, or hybrid",
    default="dct",
)
...
parser.add_argument(
    "--write_rgb",
    action="store_true",
    help="Write soft segmentation to dis as RGB file.",
)
parser.add_argument(
    "--write_bias_corrected",
    action="store_true",
    help="Write bias field corrected image to disk",
)
...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="(optional) Skipping factor to easy memory requirements of priors when estimating Gaussian parameters. Default is 1.",
)
parser.add_argument(
    "--resolution",
    type=float,
    default=0.4,
    help="(optional) Resolution of output segmentation",
)
...
parser.add_argument(
    ...
    type=float,
    default=1.00,
    help="(optional) Parameter of Greedy FireANTs registration",
)
parser.add_argument(
    "--smooth_warp_sigma",
    type=float,
...
parser.add_argument(
    "--rel_weight_labeldiff",
    type=float,
    default=2.5,
    help="(optional) Relative weight of labels in Greedy FireANTs registration",
)
parser.add_argument(
    "--save_atlas_nonlinear_reg",
    action="store_true",
    help="Save nonlinear atlas registration",
)
...
if synthseg_exists:
    ...
    if np.sum(tmp > 1000) == 0:
        raise Exception(
            "Segmentation does not include parcellation! Please use different file or re-run SynthSeg with --parc"
        )
else:
...
if np.sum(np.abs(aff - FIELDaff)) > 1e-6:
    raise Exception(
        "affine matrix of deformation field does not coincide with that of input"
    )
    sys.exit(1)
...
aff[:3, -1] = aff[:3, -1] + aff[:-1, :-1] @ np.array(
    [
        cropping[0].detach().cpu().numpy(),
        cropping[1].detach().cpu().numpy(),
        cropping[2].detach().cpu().numpy(),
    ]
)
...
tissue_index, grouping_labels, label_list, number_of_gmm_components = (
    relab.get_tissue_settings(
        os.path.join(BASE_PATH, "data_simplified", "atlas_names_and_labels.yaml"),
        os.path.join(
            BASE_PATH, "data_simplified", "combined_atlas_labels_fireants.yaml"
        ),
        os.path.join(
            BASE_PATH, "data_simplified", "combined_aseg_labels_fireants.yaml"
        ),
...
label_loader = DataLoader(
    LabelDataset(atlas_names), num_workers=workers, prefetch_factor=prefetch_factor
)
...
for n, (prior_indices, prior_values) in enumerate(label_loader):
    ...
    if prior_indices.numel() == 0:
        continue
    prior_indices = torch.as_tensor(
        prior_indices, device=device, dtype=torch.long
    ).squeeze()
    prior_values = torch.as_tensor(prior_values, device=device, dtype=dtype).squeeze()
    idx = tissue_index[n]
    if n == 0:
        prior = torch.sparse_coo_tensor(
            prior_indices[None], prior_values, [torch.Size(atlas_size).numel()]
        ).to_dense()
        ...
        prior = torch.sparse_coo_tensor(
            prior_indices, prior_values, crop_atlas_size
        ).to_dense()
        ...
        if np.any(
            np.array([343, 368, 372, 408, 418, 562, 566, 571, 339, 354])
            == label_list[n]
        ):
        ...
        if np.any(
            np.array([215, 216, 377, 217, 214, 242, 301, 238, 240, 277, 278, 279])
            == label_list[n]
        ):
...
Ifake, aff_fake = my.torch_resize(
    Ifake,
    aff_A,
    native_resolution,
    device,
    dtype=dtype,
    power_factor_at_half_width=(0.5 * 5.0),
)
...
fakeCOGvox = (
    torch.linalg.inv(torch.tensor(aff_fake, device=device, dtype=dtype)) @ fakeCOGras
)
...
II, JJ, KK = np.meshgrid(
    np.arange(Iim.shape[0]),
    np.arange(Iim.shape[1]),
    np.arange(Iim.shape[2]),
    indexing="ij",
)
...
if side == "left":
    ...
    groups = [
        ...
        [49],
        [52],
        [0, 43, 44, 14, 15, 24, 2, 3, 4, 5, 7, 8, 10, 11, 12, 13, 17, 18, 26, 28, 161],
        [54],
    ]
for group in groups:
...
affine = (
    np.linalg.inv(aff_A)
    @ my.getM(imageCOGvox[:-1, :], fakeCOGras[:-1, :]).detach().cpu().numpy()
)
II2 = affine[0, 0] * II + affine[0, 1] * JJ + affine[0, 2] * KK + affine[0, 3]
JJ2 = affine[1, 0] * II + affine[1, 1] * JJ + affine[1, 2] * KK + affine[1, 3]
KK2 = affine[2, 0] * II + affine[2, 1] * JJ + affine[2, 2] * KK + affine[2, 3]
...
for framelist in frame_sets:
    ...
    for frame in framelist:
        ...
        if frame == 2:
            M += my.fast_3D_interp_torch(
                A[..., frame]
                + torch.tensor(MLhippo - AMYGDALA, device=device, dtype=dtype),
                II2,
                JJ2,
                KK2,
                "linear",
            ...
            M += my.fast_3D_interp_torch(
                A[..., frame]
                + torch.tensor(CLAUSTRUM + RETICULAR, device=device, dtype=dtype),
                II2,
                JJ2,
                KK2,
                "linear",
            ...
            M += my.fast_3D_interp_torch(
                A[..., frame] - torch.tensor(CLAUSTRUM, device=device, dtype=dtype),
                II2,
                JJ2,
                KK2,
                "linear",
            ...
            M += my.fast_3D_interp_torch(
                A[..., frame]
                - torch.tensor(LGN + RETICULAR, device=device, dtype=dtype),
                II2,
                JJ2,
                KK2,
                "linear",
            ...
            M += my.fast_3D_interp_torch(
                A[..., frame], II2, JJ2, KK2, "linear", pad_value=pad
            )
    image2.array = torch.cat(
        [image2.array, M.permute([2, 1, 0])[None, None, ...]], dim=1
    )
# Amygdala is a bit special because it's on its own
M = my.fast_3D_interp_torch(
    torch.tensor(AMYGDALA, device=device, dtype=dtype),
    II2,
    JJ2,
    KK2,
    "linear",
...
reg = GreedyRegistration(
    ...
    fixed_images=batch1,
    moving_images=batch2,
    loss_type="custom",
    custom_loss=HybridDiceLabelDiffloss(
        kernel_size=args.cc_kernel_size, rel_weight_labeldiff=args.rel_weight_labeldiff
    ),
    ...
    reduction="mean",
)  # crucial option, for proper display of loss...
...
for t in range(len(number_of_gmm_components) - 1):
    ...
    if len(x) > 0:
        ...
        if number_of_gmm_components[t + 1] == 1:
            ...
            nx = len(x)
            gmm_mus = torch.linspace(
                mu - 0.5 * std, mu + 0.5 * std, nc, dtype=dtype, device=device
            )
            ...
            for its in range(200):
                ...
                for c in range(nc):
                    W[:, c] = (
                        gmm_ws[c]
                        / torch.sqrt(2.0 * torch.pi * torch.sqrt(gmm_var))
                        * torch.exp(-0.5 * (x - gmm_mus[c]) ** 2 / gmm_var)
                    )
...
wc_r, _ = my.torch_resize(
    warped_coords[0].permute([2, 1, 0, 3]), aff, resolution, device, dtype=dtype
)
del warped_coords
...
W = torch.zeros(
    [*priors.shape[:-1], number_of_gmm_components.sum()], dtype=dtype, device=device
)
...
for em_it in range(100):
    ...
    for c in range(n_tissues):
        ...
        for g in range(num_components):
            ...
            W[:, :, :, gaussian_number] = (
                weights[gaussian_number]
                * prior
                * torch.exp(-d * d / (2 * variances[gaussian_number]))
                / torch.sqrt(2.0 * torch.pi * variances[gaussian_number])
            )
    ...
    for c in range(number_of_gmm_components.sum()):
        ...
        if c > 0:
            ...
            d = x - means[c]
            variances[c] = (
                torch.sum(d * d * W[:, :, :, c]) + prior_count * prior_variance
            ) / (norm + prior_count + 2)
        v = variances[c]
        prior_loglhood = prior_loglhood - (
            (1 + 0.5 * prior_count) * torch.log(v)
            + 0.5 * prior_count * prior_variance / v
        ) / torch.numel(normalizer)
    loglhood = loglhood + prior_loglhood.detach().cpu().numpy()

    mixture_weights = torch.sum(
        W[:, :, :, 1:].reshape(
            [np.prod(priors.shape[:-1]), number_of_gmm_components.sum() - 1]
        )
        + 1e-9,
        axis=0,
    )
    for c in range(n_tissues - 1):
        ...
        gaussian_numbers = torch.tensor(
            np.sum(number_of_gmm_components[1 : c + 1])
            + np.array(range(num_components)),
            device=device,
            dtype=dtype,
        ).long()

        mixture_weights[gaussian_numbers] /= torch.sum(
            mixture_weights[gaussian_numbers]
        )
    ...
    print(
        "         Step %d of EM, -loglhood = %.6f" % (em_it + 1, -loglhood), flush=True
    )
...
GAUSSIAN_LHOODS = torch.zeros(
    [*I_r.shape, sum(number_of_gmm_components)], dtype=dtype, device=device
)
for c in range(sum(number_of_gmm_components)):
    # The 1e-9 ensures no zeros were prior is not zero
    GAUSSIAN_LHOODS[..., c] = (
        1.0
        / torch.sqrt(2 * math.pi * variances[c])
        * torch.exp(-0.5 * torch.pow(I_r - means[c], 2.0) / variances[c])
        + 1e-9
    )
...
for n, (prior_indices, prior_values) in enumerate(label_loader):
    ...
    if n == 0:
        ...
        Irescaled = I * ((Ashape[0] - 1) / (max_x - min_x)) + (
            (Ashape[0] - 1 - min_x - max_x) / (max_x - min_x)
        )
        Jrescaled = J * ((Ashape[1] - 1) / (max_y - min_y)) + (
            (Ashape[1] - 1 - min_y - max_y) / (max_y - min_y)
        )
        Krescaled = K * ((Ashape[2] - 1) / (max_z - min_z)) + (
            (Ashape[2] - 1 - min_z - max_z) / (max_z - min_z)
        )
        mask = Irescaled >= (-1)
        mask &= Irescaled <= 1
        mask &= Jrescaled >= (-1)
        ...
        if mask.any() == False:
            ...
            del tmp, mask
            lr_crop = (
                slice(lr_min_x, lr_max_x),
                slice(lr_min_y, lr_max_y),
                slice(lr_min_z, lr_max_z),
            )

    if skip_this_label == False:

        prior_resampled = grid_sample(
            prior[None, None, ...],
            torch.stack(
                [Krescaled[lr_crop], Jrescaled[lr_crop], Irescaled[lr_crop]], axis=-1
            )[None, ...],
            align_corners=True,
        )[0, 0, ...]
        if n == 0:  # background
        ...
        gaussian_numbers = torch.tensor(
            np.sum(number_of_gmm_components[: tissue_index[n]])
            + np.array(range(num_components)),
            device=device,
            dtype=dtype,
        ).long()
        lhood = torch.sum(
            GAUSSIAN_LHOODS[:, :, :, gaussian_numbers]
            * weights[None, None, None, gaussian_numbers],
            3,
        )
...
my.MRIwrite(
    seg.detach().cpu().numpy().astype(np.uint16),
    aff_r,
    output_dir + "/seg." + side + ".nii.gz",
)
if args.write_rgb:
    my.MRIwrite(
        seg_rgb.detach().cpu().numpy().astype(np.uint8),
        aff_r,
        output_dir + "/seg." + side + ".rgb.nii.gz",
    )
...
cmd = (
    cmd + " -v " + output_dir + "/seg." + side + ".nii.gz:colormap=lut:lut=" + LUT_file
)
if args.write_rgb:
