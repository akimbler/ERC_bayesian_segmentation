...
parser.add_argument(
    "--i_field",
    help="Registration to data/mni.nii.gz provided by mri_easyreg. Will be computed if if does not exist",
)
...
parser.add_argument(
    "--gmm_mode", help="hemisphere to segment (must be l or r)", default="1mm"
)
parser.add_argument(
    "--bf_mode",
...
parser.add_argument(
    "--line_search",
    default="Armijo",
    help="type of line search; must be Armijo (default) or Wolfe",
)
...
parser.add_argument(
    "--o_bf_corr", help="Bias field corrected volume, in .mgz or .nii(.gz) format"
)
...
parser.add_argument(
    ...
    type=float,
    default=4.5,
    help="(optional) Weight of regularized of atlas deformation",
)
...
if os.path.isfile(input_seg):
    ...
    if (output_synthseg_vols is None) or (os.path.isfile(output_synthseg_vols)):
        ...
        print(
            "However, I did not find the CSV file with the volumes; I need to rerun SynthSeg, sorry!"
        )
...
tissue_index, grouping_labels, label_list, number_of_gmm_components = (
    relab.get_tissue_settings(
        os.path.join(
            BASE_PATH, "data_" + args.atlas_mode, "atlas_names_and_labels.yaml"
        ),
        os.path.join(
            BASE_PATH,
            "data_" + args.atlas_mode,
        ...
        os.path.join(
            BASE_PATH,
            "data_" + args.atlas_mode,
            "gmm_components_" + args.gmm_mode + ".yaml",
        ),
        aseg_label_list,
    )
)
        ...
        if number_of_gmm_components[t + 1] == 1:
            ...
            for its in range(200):
                ...
                for c in range(nc):
                    W[:, c] = (
                        gmm_ws[c]
                        / np.sqrt(2.0 * np.pi * np.sqrt(gmm_var))
                        * np.exp(-0.5 * (x - gmm_mus[c]) ** 2 / gmm_var)
                    )
...
for n in range(n_levels):
    ...
    slow = (
        device.type == "cpu"
        or torch.cuda.get_device_properties(device).total_memory < 30 * 1024**3
    )
    A_r, aff_A_r = my.torch_resize(
        A, aff_A, RESOLUTION_LEVELS[n], device, dtype=dtype, slow=slow
    )
    ...
    for flexibility_mode in range(3):
        ...
        if (
            (allow_similarity == False)
            and (allow_shearing == False)
            and (allow_nonlinear == False)
        ):
        ...
        for outter_it in range(outter_its):
            ...
            denominator = np.prod(
                I_r[:: VOXEL_SKIP[n], :: VOXEL_SKIP[n], :: VOXEL_SKIP[n]].shape
            )
            prior_loglhood = (
                -(
                    (1 + 0.5 * prior_count) * np.log(var_bg)
                    + 0.5 * prior_count * prior_variance / var_bg
                )
                / denominator
            )
            for c in range(len(vars)):
                prior_loglhood -= (
                    (1 + 0.5 * prior_count) * np.log(vars[c])
                    + 0.5 * prior_count * prior_variance / vars[c]
                ) / denominator

            print(
                "   Deforming atlas: outter iteration "
                + str(outter_it + 1)
                + " of "
                + str(outter_its)
            )

            model = nets.AtlasDeformer(
                I_r,
                aff_r,
                A_r,
                aff_A_r,
            ...
            optimizer = FullBatchLBFGS(
                model.parameters(), lr=LR, line_search=line_search
            )
            ...
            for inner_it in range(inner_its):
                ...
                print(
                    "         Step %d of atlas deformation, loss = %.6f"
                    % (inner_it + 1, loss_np),
                    flush=True,
                )
            ...
            if allow_gaussian_update:
                with torch.no_grad():
                    print(
                        "   Updating Gaussian parameters with EM: outter iteration "
                        + str(outter_it + 1)
                        + " of "
                        + str(outter_its)
                    )
                    ...
                    variances = torch.tensor(
                        [var_bg, *vars], device=device, dtype=dtype
                    )
                    weights = torch.tensor(
                        np.array([1.0] + mixture_weights.tolist()),
                        device=device,
                        dtype=dtype,
                    )
                    priors = torch.tensor(priors, device=device, dtype=dtype)
                    W = torch.zeros(
                        [*priors.shape[:-1], number_of_gmm_components.sum()]
                    ).to(device)
                    x = torch.tensor(
                        I_r[:: VOXEL_SKIP[n], :: VOXEL_SKIP[n], :: VOXEL_SKIP[n]],
                        device=device,
                        dtype=dtype,
                    )
                    ...
                    prior_count = (
                        100 / (RESOLUTION_LEVELS[n] ** 3) / (VOXEL_SKIP[n] ** 3)
                    )
                    prior_variance = var_bg
                    loglhood_old = -10000
                    for em_it in range(10):
                        ...
                        for c in range(n_tissues):
                            ...
                            for g in range(num_components):
                                ...
                                W[:, :, :, gaussian_number] = (
                                    ...
                                    * prior
                                    * torch.exp(
                                        -d * d / (2 * variances[gaussian_number])
                                    )
                                    / torch.sqrt(
                                        2.0 * math.pi * variances[gaussian_number]
                                    )
                                )

                        normalizer = 1e-9 + torch.sum(W, dim=-1, keepdim=True)
                        loglhood = (
                            torch.mean(torch.log(normalizer)).detach().cpu().numpy()
                        )
                        W = W / normalizer
                        ...
                        mixture_weights = torch.sum(
                            W[:, :, :, 1:].reshape(
                                [
                                    np.prod(priors.shape[:-1]),
                                    number_of_gmm_components.sum() - 1,
                                ]
                            )
                            + 1e-9,
                            axis=0,
                        ...
                        print(
                            "         Step %d of EM, -loglhood = %.6f"
                            % (em_it + 1, -loglhood + deformation_penalty),
                            flush=True,
                        )
...
with torch.no_grad():
    ...
    for n, (prior_indices, prior_values) in enumerate(label_loader):
        print(
            "Reading in label " + str(n + 1) + " of " + str(n_labels),
            end="\r",
            flush=True,
        )
        ...
        prior_values = torch.as_tensor(
            prior_values, device=device, dtype=dtype
        ).squeeze()
        ...
        if n == 0:
            ...
            prior_indices = my.ind2sub(prior_indices, atlas_size)
            min_x, max_x = (
                prior_indices[0].min().item(),
                prior_indices[0].max().item() + 1,
            )
            min_y, max_y = (
                prior_indices[1].min().item(),
                prior_indices[1].max().item() + 1,
            )
            min_z, max_z = (
                prior_indices[2].min().item(),
                prior_indices[2].max().item() + 1,
            )
            crop_atlas_size = [max_x - min_x, max_y - min_y, max_z - min_z]
            prior_indices[0] -= min_x
            prior_indices[1] -= min_y
            ...
            min_x, min_y, min_z = (
                hr2lr[:3, :3]
                @ np.asarray([min_x - 1, min_y - 1, min_z - 1] + hr2lr[:3, -1])
            ).tolist()
            max_x, max_y, max_z = (
                hr2lr[:3, :3] @ np.asarray([max_x, max_y, max_z] + hr2lr[:3, -1])
            ).tolist()
        ...
        if RESOLUTION_LEVELS[-1] != RESOLUTION_ATLAS:
            prior, aff_pr = my.torch_resize(
                prior, aff_pr, RESOLUTION_LEVELS[-1], device, dtype=dtype
            )

        # shift/scale sampling grid appropriately
        aff_shift = torch.as_tensor(
            np.linalg.inv(aff_pr) @ aff_A_r, device=device, dtype=dtype
        )
        grids_shifted = grids[(slice(None), *lr_crop, slice(None))]
        grids_shifted = (
            aff_shift[:3, :3].matmul(grids_shifted.unsqueeze(-1)).squeeze(-1)
        )
        grids_shifted = grids_shifted.add_(aff_shift[:3, -1])

        prior = interpol.grid_pull(
            prior[None, None, :, :, :], grids_shifted, interpolation=1
        )
        del grids_shifted
